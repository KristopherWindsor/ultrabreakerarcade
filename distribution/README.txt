
****************************************
*                                      *
*  %%  %%  %%%%                        *
*  %%  %%  %%  %%              x       *
*  %%  %%  %%%%                        *
*  %%  %%  %%  %%                      *
*  %%%%%%  %%%%                        *
*                                      *
*                                      *
*                     x                *
*                  -------             *
****************************************

---------- Ultrabreaker

http://ultrabreaker.com/
It's the new brick-breaking game in town.

---------- How to play

Thanks for playing!
For help, please see the welcome message:
http://ultrabreaker.com/forum/index.php?topic=16

---------- Credits

Design, development, and graphics:
	Kristopher Windsor

Beta testing:
	Kristian, Kaleb, and Kristof Windsor

Code library development:
	Cha0s (Chisock)
	Dr_D and Yetifoot (Rotozoom)
	D.J. Peters (AVICapture)
	Victor Aurion Moentero Leonheart (ABFont)
	Mysoft (ASM optimization)
	Yetifoot (FBPNG)
	The FreeBASIC dev team (FreeBASIC compiler)

Music:
	???????????????????????????????????????????????????????????????????????????????????

UltraBreaker is (C) Copyright 2006 - 2010 Kristopher Windsor
All libraries and assets used with permission

---------- Code info and licenses

ABFont:
http://www.gzip.org/zlib/zlib_license.html

AVICapture:
http://www.freebasic.net/forum/viewtopic.php?t=13066

ChiSock:
http://www.freebasic.net/forum/viewtopic.php?t=8454

FBGFX:
http://fbc.svn.sourceforge.net/viewvc/fbc/trunk/FreeBASIC/src/gfxlib2/

FBPNG:
http://www.freebasic.net/forum/viewtopic.php?t=8024

FMOD:
http://www.fmod.org/index.php/sales/

iGUI:
http://www.freebasic.net/forum/viewtopic.php?t=10364

Rotozoom:
http://www.freebasic.net/forum/viewtopic.php?t=13251

ZLib:
http://www.zlib.net/zlib_license.html

---------- Support

Need help? Have suggestions that you don't want to post on the forum?
Email is appreciated: support@ultrabreaker.com
